...
%   voxels      vector [x y] of dimensions of output radiation
    ...
    for x = 1:voxels(1)
        for y = 1:voxels(2)
              radiation(x,y) = amplitude * exp(noiselevel*randn) * exp(-1/2*( ...
                  (x-mean(1))^2/variance(1,1)/(1-variance(1,2)*variance(2,1)/variance(1,1)/variance(2,2)) + ...
                  (y-mean(2))^2/variance(2,2)/(1-variance(1,2)*variance(2,1)/variance(1,1)/variance(2,2)) + ...
                  2*(x-mean(1))*(y-mean(2))/(variance(1,2)*variance(2,1)-variance(1,1)*variance(2,2))) );
        end
    end
